package com.dhq.mywidget.layoutmanger;
 ...
 */
public class SimpleLayoutManager extends RecyclerView.LayoutManager implements RecyclerView.SmoothScroller.ScrollVectorProvider{
    ...
    private boolean mIsCanCycle = true;
    private Context mContext;

    /**
     * @param context
     * @param isCanCycle 是否可以循环滑动
     */
    public SimpleLayoutManager(Context context, boolean isCanCycle) {
        this.mContext = context;
        this.mIsCanCycle = isCanCycle;
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        ensureStatus();
        int offset = 0;
        // 计算RecyclerView 可用布局宽度 具体实现 mLayoutManager.getWidth() - mLayoutManager.getPaddingLeft() - mLayoutManager.getPaddingRight();
    ...
    private int fillChild(RecyclerView.Recycler recycler, int dx, RecyclerView.State state) {
        if (dx > 0) {
            ...
            if (offset - dx < getWidth() - getPaddingRight()) {
                // item 足够
                if (mIsCanCycle) {
                    int realPosition = position == getItemCount() ? 0 : position;
                    layoutScrapRight(recycler, realPosition, offset);
                } else {
            ...
            if (offset - dx > getPaddingLeft()) {
                ...
                if (mIsCanCycle) {
                    int realPosition = position == -1 ? getItemCount() - 1 : position;
                    layoutScrapleft(recycler, realPosition, offset);
                } else {
                    if (position >= 0) {
    ...
    private void recylerUnVisiableView(RecyclerView.Recycler recycler, int dx) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (dx > 0) {
                ...
                //RecyclerView从左往右滑动时 //将右边的childView 回收掉
     ...
     * 将可见区域的childView layout出来
     */
    private void layoutScrap(RecyclerView.Recycler recycler, RecyclerView.State state,
                             int offset, int mAvailable) {

        int itemCount = state.getItemCount();
        int index = mPendingScrollPosition;
        while (true) {
            if (mAvailable <= 0) {
                break;
            }
            int position = index % itemCount;
            //在右边添加新的childView
            int childWidth = layoutScrapRight(recycler, position, offset);
     ...
     */
    private int layoutScrap(RecyclerView.Recycler recycler, int position, int offset,
                            int direction) {

    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        if (mIconX + mDistanceX <= 0) { // 控制SlideIcon不能超过左边界限
            mSlideIcon.layout(
                    0,
                    0,
                    mSlideIcon.getMeasuredWidth(),
                    mSlideIcon.getMeasuredHeight());
        } else if (mIconX + mDistanceX + mSlideIcon.getMeasuredWidth() >= getMeasuredWidth()) { // 控制SlideIcon不能超过左边界限
            mSlideIcon.layout(
                    getMeasuredWidth() - mSlideIcon.getMeasuredWidth(),
                    0,
                    getMeasuredWidth(),
                    mSlideIcon.getMeasuredHeight());
            ...
            mSlideIcon.layout(
                    mIconX + mDistanceX,
                    0,
                    mIconX + mDistanceX + mSlideIcon.getMeasuredWidth(),
                    mSlideIcon.getMeasuredHeight());
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIconX + mDistanceX > 0) {
            ...
            RectF rectF = new RectF(0,
                    0,
                    mIconX + mDistanceX + mSlideIcon.getMeasuredHeight() / 2,
                    getMeasuredHeight());

...
//        mViewPager.setLayoutParams(vParams);
     ...
     */
    public void setOnBannerListener(BannerListener<T> itemClickListener) {
        this.mBannerListener = itemClickListener;
    }
    ...
    private class MyAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
                        ...
                        case MotionEvent.ACTION_UP:
                            ...
                            if (downX == moveX && (moveTime - downTime < 500)) {//点击的条件
                                ...
                                if (mBannerListener != null) {
                                    mBannerListener.itemClick(mUrlList.get(position % mUrlList.size()), position % mUrlList.size());
                                }
                            }
                            break;
            ...
            //加载图片
            if (mBannerListener != null) {
                mBannerListener.getView(iv, mUrlList.get(position % mUrlList.size()), position % mUrlList.size());
            }
    ...
    public void onPageSelected(int position) {
        ...
        if (mBannerListener != null) {
            mBannerListener.selectitem(mUrlList.get(position % mUrlList.size()), position % mUrlList.size());
        }
    }
    ...
    public interface BannerListener<T> {
        void getView(ImageView imageView, T data, int position);

        void selectitem(T data, int position);
